...
def test_log_command_shows_messages_for_session(tmp_path: Path) -> None:
    ...
    assert "GGbot log" in res.stdout
    assert "[USER] hello" in res.stdout
    assert "[ASSISTANT] hi there" in res.stdout
...
def test_log_command_includes_runtime_events_by_default(tmp_path: Path) -> None:
    ...
    assert res.exit_code == 0
    assert "[EVENT] tool_call name=workspace_list" in res.stdout
    assert "[EVENT] tool_result name=workspace_list status=ok" in res.stdout
