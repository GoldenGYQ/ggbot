    ...
    return [
        ...
        PromptLayer(
            ...
            template=(
                ...
                " Search Tool Selection Recommendations\n"
                "Choose the appropriate search tool based on task requirements:\n"
                "1. When you need real webpage content: Use `news_search` (set `fetch_top_k: 3-5`)\n"
                "   - Examples: searching for latest news, technical documentation, market reports\n"
                "   - Note: requires specifying number of pages to fetch\n"
                "\n"
                "2. When you need reliable search results: Use `enhanced_search`\n"
                "   - Automatically tries multiple search engines\n"
                "   - Falls back to relevant simulated data when search fails\n"
                "   - Suitable for: market data, technical information, general queries\n"
                "\n"
                "3. When you need webpage content previews: Use `search_with_content`\n"
                "   - Searches and fetches webpage content snippets\n"
                "   - Suitable for scenarios where webpage content needs to be viewed\n"
                "\n"
                "4. For simple link searches: Use `duckduckgo_search`\n"
                "   - Returns only links and snippets\n"
                "   - Note: some queries may return empty results\n"
                "\n"
